...
match bmi160.read_all() {
    Ok(data) => {
        ...
        let imu_data_x: &str = imu_buffer_x.format(data.gyro.x ); //  + data.gyro.y + data.gyro.z)
        Text::with_baseline(imu_data_x, Point::new(0, 0), text_style, Baseline::Top)
        .draw(&mut display)
        .unwrap();
    let imu_data_y: &str = imu_buffer_y.format(data.gyro.y ); //  + data.gyro.y + data.gyro.z)
    Text::with_baseline(imu_data_y, Point::new(40, 0), text_style, Baseline::Top)
    .draw(&mut display)
    .unwrap();
let imu_data_z: &str = imu_buffer_z.format(data.gyro.z ); //  + data.gyro.y + data.gyro.z)
Text::with_baseline(imu_data_z, Point::new(80, 0), text_style, Baseline::Top)
.draw(&mut display)
.unwrap();
...
Line::new(Point::new(0, -0), Point::new(105 ,-0))
.into_styled(PrimitiveStyle::with_stroke(BinaryColor::On, 1))
.translate(Point::new(0 ,15))
.draw(&mut display)
.unwrap();
Line::new(Point::new(105, -0), Point::new(105 ,15))
.into_styled(PrimitiveStyle::with_stroke(BinaryColor::On, 1))
