...
spawner.must_spawn(reset_memory(Input::new(peripherals.GPIO4, InputConfig::default().with_pull(Pull::Up))));
    ...
    let clock_cfg: PeripheralClockConfig = PeripheralClockConfig::with_frequency(Rate::from_mhz(32)).unwrap();
    ...
    let mut pmv_pin_b = mcpwm.operator1.with_pin_a(peripherals.GPIO32, PwmPinConfig::UP_ACTIVE_HIGH);
    ...
    spawner.must_spawn(lib::servo::servo_test(pwm_pin,pmv_pin_b));
